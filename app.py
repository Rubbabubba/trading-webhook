...
PATCH_VERSION = "patch-174.3-kpi-layout-overlap-fix"
...
def dashboard(request: Request):
    ...
    active_rows = "".join(
        ...
        f"<td>{_dashboard_fmt(_snapshot_value(r['last']))}</td>"
...
<div class="section grid"><div class="card"><h2>Rejected by Reason (last {rejection_scan_window} scans)</h2><table><thead><tr><th>Reason</th><th>Count</th></tr></thead><tbody>{rejection_window_html}</tbody></table></div><div class="card"><h2>Would-have-entered (if only correlation relaxed)</h2><table>{_rows([('raw_rows_in_window', raw_relaxed_count), ('unique_symbols_blocked', unique_relaxed_count), ('estimated_unblock_rate_pct', f'{relaxed_unblock_rate}%')])}</table><h3 style="margin-top:12px;">Correlation Profitability Proxy</h3><table>{_rows([('closed_trades_on_relaxed_symbols', correlation_proxy_closed), ('wins_on_relaxed_symbols', correlation_proxy_wins), ('win_rate_pct', f'{correlation_proxy_win_rate}%'), ('gross_pnl_on_relaxed_symbols', _money(correlation_proxy_pnl))])}</table><table style="margin-top:10px;"><thead><tr><th>Symbol</th><th>Rank</th><th>Close</th><th>Blocking reasons</th></tr></thead><tbody>{relaxed_only_html}</tbody></table><div class="muted" style="margin-top:10px;">Preview deduped by symbol, capped at 10 rows, and wrapped for narrow cards.</div></div></div>
