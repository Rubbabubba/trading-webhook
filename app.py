...
INTRADAY_VWAP_RECLAIM_MIN_RANK_SCORE = float(getenv_any("INTRADAY_VWAP_RECLAIM_MIN_RANK_SCORE", default="80"))
INTRADAY_VWAP_CONTINUATION_MIN_RANK_SCORE = float(getenv_any("INTRADAY_VWAP_CONTINUATION_MIN_RANK_SCORE", default="80"))
...
INTRADAY_VWAP_CONTINUATION_ENABLE = env_bool("INTRADAY_VWAP_CONTINUATION_ENABLE", "true")
INTRADAY_VWAP_CONTINUATION_MIN_1M_BARS = int(os.getenv("INTRADAY_VWAP_CONTINUATION_MIN_1M_BARS", "25"))
INTRADAY_VWAP_CONTINUATION_MIN_5M_BARS = int(os.getenv("INTRADAY_VWAP_CONTINUATION_MIN_5M_BARS", "5"))
INTRADAY_VWAP_CONTINUATION_SCORE_MIN = float(os.getenv("INTRADAY_VWAP_CONTINUATION_SCORE_MIN", "62"))
INTRADAY_VWAP_CONTINUATION_MAX_EXTENSION_PCT = float(os.getenv("INTRADAY_VWAP_CONTINUATION_MAX_EXTENSION_PCT", "0.012"))
INTRADAY_VWAP_CONTINUATION_MIN_RECENT_VOL_RATIO = float(os.getenv("INTRADAY_VWAP_CONTINUATION_MIN_RECENT_VOL_RATIO", "0.75"))
INTRADAY_VWAP_CONTINUATION_MIN_DAY_RANGE_PCT = float(os.getenv("INTRADAY_VWAP_CONTINUATION_MIN_DAY_RANGE_PCT", "0.003"))
INTRADAY_VWAP_CONTINUATION_ALLOW_NEGATIVE_VWAP_SLOPE_PCT = float(os.getenv("INTRADAY_VWAP_CONTINUATION_ALLOW_NEGATIVE_VWAP_SLOPE_PCT", "-0.0005"))
...
def _dashboard_scanner_status_view(
    ...
    if status in unhealthy_statuses or event in unhealthy_events:
        ...
        health = "healthy"
        ready = True
    elif in_flight or status in {"running", "in_flight", "started", "scan_start"} or event in {"scan_start"}:
        health = "in_flight"
        ready = True
    elif status in {"received"} or event in {"received", "scan_request"}:
        health = "healthy"
        ready = True
    else:
        health = "unknown"
...
def _dashboard_readiness_assessment(blockers: object, scanner_ready: object, launch_blockers: object = None) -> dict:
    blocker_list = list(blockers or []) if isinstance(blockers, (list, tuple, set)) else ([blockers] if blockers else [])
    launch_blocker_list = list(launch_blockers or []) if isinstance(launch_blockers, (list, tuple, set)) else ([launch_blockers] if launch_blockers else [])
    system_ready = not bool(blocker_list) and bool(scanner_ready)
    launch_ready = system_ready and not bool(launch_blocker_list)
    if launch_ready:
        status = "READY"
        css_class = "good"
    elif system_ready:
        status = "LAUNCH SETUP NEEDED"
        css_class = "neutral"
    else:
        status = "CHECK BLOCKERS"
        css_class = "neutral"
    return {
        "status": status,
        "class": css_class,
        "system_ready": system_ready,
        "intraday_launch_ready": launch_ready,
        "blocker_count": len(blocker_list),
        "launch_blocker_count": len(launch_blocker_list),
    }
...
def _intraday_launch_gate_actions(launch_blockers: object) -> list[str]:
    ...
    actions = []
    if {"market_not_tradable_now", "market_tradable_now"} & blockers:
        actions.append("wait_for_market_open")
    if {"regime_not_favorable", "regime_favorable"} & blockers:
        actions.append("wait_for_regime_favorable")
    if {"intraday_max_open_positions_not_above_base", "projected_intraday_open_slots_below_launch_min", "intraday_capacity_override_above_base", "projected_open_slots_meet_launch_min"} & blockers:
        actions.append("apply_intraday_capacity_env")
    ...
    for p in positions:
        try:
            ...
            try:
                ...
                avg_entry = None
            try:
                current_price = float(getattr(p, "current_price", None) or 0) or None
            except Exception:
                current_price = None
            try:
                market_value = float(getattr(p, "market_value", None) or 0) or None
            except Exception:
                market_value = None
            try:
                unrealized_pl = float(getattr(p, "unrealized_pl", None) or 0)
            except Exception:
                unrealized_pl = None
            try:
                unrealized_plpc = float(getattr(p, "unrealized_plpc", None) or 0)
            except Exception:
                unrealized_plpc = None
            row = {"symbol": sym, "qty": qty, "avg_entry_price": avg_entry}
            ...
            if unrealized_plpc is not None:
                row["unrealized_plpc"] = unrealized_plpc
            out.append(row)
        except Exception:
            continue
    return out
...
def _scanner_min_rank_for_signal(signal_name: str, family: str = "primary") -> float:
    sig = str(signal_name or "").upper()
    if sig == "INTRADAY_VWAP_RECLAIM":
        return float(INTRADAY_VWAP_RECLAIM_MIN_RANK_SCORE)
    if sig == "INTRADAY_VWAP_CONTINUATION":
        return float(INTRADAY_VWAP_CONTINUATION_MIN_RANK_SCORE)
    return float(SCANNER_FALLBACK_MIN_RANK_SCORE if str(family or "") == "fallback" else SCANNER_RANK_MIN_SCORE)
...
def _intraday_launch_projection(open_count_override: int | None = None) -> dict:
    mode = str(globals().get("STRATEGY_MODE") or getenv_any("STRATEGY_MODE", default="")).strip().lower()
    ...
    daily_stop_multiplier = max(1.0, float(getenv_float_any("INTRADAY_LAUNCH_DAILY_STOP_MULTIPLIER", default=2.0) or 2.0))
    daily_loss_multiplier = max(1.0, float(getenv_float_any("INTRADAY_LAUNCH_DAILY_LOSS_MULTIPLIER", default=2.0) or 2.0))
    daily_stop_floor = max(float(base_daily_stop), float(getenv_float_any("INTRADAY_LAUNCH_BASE_DAILY_STOP_DOLLARS", default=150.0) or 150.0))
    daily_loss_floor = max(float(base_daily_loss), float(getenv_float_any("INTRADAY_LAUNCH_BASE_DAILY_LOSS_LIMIT", default=200.0) or 200.0))
    recommended_intraday_daily_stop = round(max(float(projected_daily_stop), daily_stop_floor * daily_stop_multiplier), 2)
    recommended_intraday_daily_loss = round(max(float(projected_daily_loss), daily_loss_floor * daily_loss_multiplier), 2)
    open_count = int(open_count_override) if open_count_override is not None else count_open_positions_allowed()
    ...
    recommended_intraday_open_slots = max(0, int(recommended_intraday_max_positions) - int(open_count))
    recommended_env = {
        "STRATEGY_MODE": "intraday",
        "INTRADAY_MAX_OPEN_POSITIONS": str(recommended_intraday_max_positions),
        "INTRADAY_LAUNCH_MIN_OPEN_SLOTS": str(min_launch_open_slots),
        "INTRADAY_DAILY_STOP_DOLLARS": f"{recommended_intraday_daily_stop:.2f}",
        "INTRADAY_DAILY_LOSS_LIMIT": f"{recommended_intraday_daily_loss:.2f}",
    }
    next_actions = []
    optional_actions = []
    blockers = []
    ...
    if intraday_max_positions <= base_max_positions:
        capacity_actions = [
            "set_INTRADAY_MAX_OPEN_POSITIONS_above_current_base",
            f"set_INTRADAY_MAX_OPEN_POSITIONS_to_at_least_{recommended_intraday_max_positions}",
        ]
        if projected_open_slots < min_launch_open_slots:
            blockers.append("intraday_max_open_positions_not_above_base")
            next_actions.extend(capacity_actions)
        else:
            optional_actions.extend(capacity_actions)
    ...
    if projected_open_slots < min_launch_open_slots:
        ...
        required_capacity_action = f"set_INTRADAY_MAX_OPEN_POSITIONS_to_at_least_{recommended_intraday_max_positions}"
        if required_capacity_action not in next_actions:
            next_actions.append(required_capacity_action)
    if intraday_daily_stop <= base_daily_stop:
        optional_actions.append("consider_raising_INTRADAY_DAILY_STOP_DOLLARS")
    if intraday_daily_loss <= base_daily_loss:
        optional_actions.append("consider_raising_INTRADAY_DAILY_LOSS_LIMIT")
    if intraday_portfolio_cap <= base_portfolio_cap:
        optional_actions.append("consider_raising_INTRADAY_MAX_PORTFOLIO_EXPOSURE_PCT")
    if intraday_symbol_cap <= base_symbol_cap:
        optional_actions.append("consider_raising_INTRADAY_MAX_SYMBOL_EXPOSURE_PCT")
    return {
        "strategy_mode": mode,
        ...
        "recommended_intraday_open_slots": recommended_intraday_open_slots,
...
def _intraday_signal_debug_from_scan(scan: dict | None) -> dict:
    ...
    return {
        ...
        "intraday_vwap_continuation": {
            ...
            "min_rank_score": float(INTRADAY_VWAP_CONTINUATION_MIN_RANK_SCORE),
            "breakdown": strategy_breakdown.get("intraday_vwap_continuation") if isinstance(strategy_breakdown.get("intraday_vwap_continuation"), list) else [],
        },
...
def _compact_scan_result(row: dict | None) -> dict:
    ...
    continuation = diagnostics.get("intraday_vwap_continuation") if isinstance(diagnostics.get("intraday_vwap_continuation"), dict) else {}
    if continuation:
        compact["intraday_vwap_continuation"] = {
            "reason": continuation.get("reason"),
            "score": continuation.get("score"),
            "dist_to_vwap_pct": continuation.get("dist_to_vwap_pct"),
...
def _intraday_vwap_continuation_setup(bars_today: list[dict], force_enable: bool = False) -> dict:
    ...
    out: dict = {
        "enabled": enabled,
        "eligible": True,
        "strategy": "intraday_vwap_continuation",
        "candidate_path": "intraday_vwap_continuation",
    }
    if not enabled:
        out["eligible"] = False
        out["reason"] = "strategy_mode_not_intraday" if str(STRATEGY_MODE or "").strip().lower() != "intraday" and not force_enable else "disabled"
    ...
    bars_5m = resample_5m(rows)
    out["bars_5m"] = len(bars_5m)
    min_5m = max(3, int(INTRADAY_VWAP_CONTINUATION_MIN_5M_BARS or 0))
    out["min_bars_5m"] = min_5m
    if len(bars_5m) < min_5m:
        out["eligible"] = False
        out["reason"] = "insufficient_5m_bars"
    ...
    slope_back = max(1, min(5, len(vwaps) - 1))
    vwap_slope = (vwaps[-1] / max(vwaps[-1 - slope_back], 1e-9) - 1.0) if len(vwaps) > slope_back else 0.0
    dist_to_vwap_pct = ((price - vwap) / max(vwap, 1e-9)) if vwap else 0.0
    price_above_vwap = bool(price >= vwap)
    price_above_fast = bool(price >= ema_fast_now * 0.998)
    ema_stack_ok = bool(ema_fast_now >= ema_slow_now * 0.998)
    slope_ok = bool(vwap_slope >= float(INTRADAY_VWAP_CONTINUATION_ALLOW_NEGATIVE_VWAP_SLOPE_PCT))
    extension_ok = bool(0.0 <= dist_to_vwap_pct <= float(INTRADAY_VWAP_CONTINUATION_MAX_EXTENSION_PCT))
    trend_ok = bool(price_above_vwap and price_above_fast and ema_stack_ok and slope_ok)

    recent_vol = _mean(volumes_1m[-5:]) if len(volumes_1m) >= 5 else _mean(volumes_1m)
    baseline_vol = _mean(volumes_1m[-20:-5]) if len(volumes_1m) >= 20 else _mean(volumes_1m[:-5])
    recent_1m_vol_ratio = recent_vol / max(baseline_vol, 1.0) if baseline_vol > 0 else 1.0
    micro_vol_ok = bool(recent_1m_vol_ratio >= float(INTRADAY_VWAP_CONTINUATION_MIN_RECENT_VOL_RATIO))
    relvol_ok = micro_vol_ok
    ...
    entry_confirm_ok = bool(closes_1m[-1] >= highs_1m[-2] * (1.0 + float(VWAP_PB_ENTRY_CONFIRM_BUFFER_PCT))) if len(highs_1m) >= 2 else True
    micro_conditions = []
    if INTRADAY_VWAP_CONTINUATION_REQUIRE_GREEN_LAST_1M:
        micro_conditions.append(last_1m_green)
    if INTRADAY_VWAP_CONTINUATION_REQUIRE_HIGHER_LOW:
        micro_conditions.append(higher_low_ok)
    if INTRADAY_VWAP_CONTINUATION_ENTRY_CONFIRM_ABOVE_PRIOR_1M_HIGH:
        micro_conditions.append(entry_confirm_ok)
    micro_confirm_ok = all(micro_conditions) if micro_conditions else True

    day_high = max(highs_1m) if highs_1m else price
    day_low = min(lows_1m) if lows_1m else price
    day_range_pct = ((day_high - day_low) / max(price, 1e-9)) if price else 0.0
    day_range_ok = bool(day_range_pct >= float(INTRADAY_VWAP_CONTINUATION_MIN_DAY_RANGE_PCT))
    atr_ok = day_range_ok

    score = 0.0
    if trend_ok: score += 20.0
    if price_above_vwap: score += 12.0
    if price_above_fast: score += 10.0
    if ema_stack_ok: score += 10.0
    if slope_ok: score += 8.0
    if extension_ok: score += 10.0
    if micro_vol_ok: score += 8.0
    if day_range_ok: score += 6.0
    if last_1m_green: score += 4.0
    if higher_low_ok: score += 4.0
    if entry_confirm_ok: score += 4.0
    score += max(0.0, min(recent_1m_vol_ratio, 2.0)) * 4.0
    score += max(0.0, 6.0 - abs(dist_to_vwap_pct) * 600.0)

    component_reasons = []
    if not price_above_vwap: component_reasons.append("price_below_vwap")
    if not price_above_fast: component_reasons.append("price_below_ema_fast")
    if not ema_stack_ok: component_reasons.append("ema_stack_fail")
    if not slope_ok: component_reasons.append("slope_fail")
    if not extension_ok: component_reasons.append("too_extended_from_vwap")
    if not micro_vol_ok: component_reasons.append("recent_1m_volume_fail")
    if not day_range_ok: component_reasons.append("day_range_too_small")
    if not micro_confirm_ok: component_reasons.append("micro_confirm_fail")
    ...
    recent_low = min(lows_1m[-max(3, min(8, len(lows_1m))):])
    stop_price = max(0.01, min(recent_low, vwap * (1.0 - 0.0025)))
    risk_per_share = max(price - stop_price, price * 0.0025)
    target_price = price + (risk_per_share * float(SWING_TARGET_R_MULT or 1.8))
    out.update({
        ...
        "near_miss": {
            "near": bool(not triggered and score >= max(0.0, float(INTRADAY_VWAP_CONTINUATION_SCORE_MIN) - 8.0)),
            "score": round(score, 3),
            "dist_to_level_pct": round(dist_to_vwap_pct * 100.0, 3),
        },
        "mode_thresholds": {
            "max_extension_pct": round(float(INTRADAY_VWAP_CONTINUATION_MAX_EXTENSION_PCT) * 100.0, 3),
            "min_recent_1m_vol_ratio": float(INTRADAY_VWAP_CONTINUATION_MIN_RECENT_VOL_RATIO),
            "min_day_range_pct": round(float(INTRADAY_VWAP_CONTINUATION_MIN_DAY_RANGE_PCT) * 100.0, 3),
            "score_min": float(INTRADAY_VWAP_CONTINUATION_SCORE_MIN),
        },
        "stop_price": round(stop_price, 4),
        "target_price": round(target_price, 4),
...
def _no_signal_from_intraday_vwap_continuation(diag: dict) -> str:
    if not diag.get("enabled"):
        return _strategy_reason_disabled()
    if diag.get("eligible") is False:
        return diag.get("reason") or "not_eligible"
    return diag.get("reason") or "no_continuation"
...
def _derive_no_signal_details(diag: dict) -> tuple[str, dict]:
    ...
    intraday_continuation = diag.get("intraday_vwap_continuation") or {}
    ...
    intraday_continuation_reason = _no_signal_from_intraday_vwap_continuation(intraday_continuation)
    hf_reason = "no_signal"
    ...
    if str(STRATEGY_MODE or "").strip().lower() == "intraday" and SCANNER_PRIMARY_STRATEGY in ("vwap", "vwap_pullback", "vwap_pullback_only"):
        strategy_order = ("intraday_vwap_reclaim", "intraday_vwap_continuation", "vwap_pullback", "midbox", "pwr")
...
def diagnostics_scanner():
    ...
    return {
        ...
        "intraday_candidate_path": {
            ...
            "intraday_vwap_continuation_enabled": bool(INTRADAY_VWAP_CONTINUATION_ENABLE),
            "intraday_vwap_continuation_score_min": float(INTRADAY_VWAP_CONTINUATION_SCORE_MIN),
            "intraday_vwap_continuation_min_recent_vol_ratio": float(INTRADAY_VWAP_CONTINUATION_MIN_RECENT_VOL_RATIO),
    ...
    *,
    ...
    return {
        ...
        "open_book_risk": {
            ...
            "max_open_positions": int(_effective_max_open_positions()),
            "total_notional": round(total_notional, 2),
            "total_risk_to_stop": round(total_risk, 2),
            "total_unrealized_pl": round(total_unrealized, 2),
            "portfolio_exposure_cap_pct": float(_effective_portfolio_exposure_cap_pct()),
            "symbol_exposure_cap_pct": float(_effective_symbol_exposure_cap_pct()),
            "rows": open_rows,
        },
...
def _swing_capacity_advisory(
    *,
    current_max: object = None,
    current_open_positions: object = None,
    closed_trades: object = None,
    win_rate: object = None,
    avg_r: object = None,
) -> dict:
    """Operator-only guidance for staged swing capacity increases.

    This helper is advisory only.  It does not change the configured trading
    limits; it summarizes whether the closed-trade sample is mature enough to
    consider raising the swing concurrent-position ceiling.
    """
    ...
    current = int(_num(current_max, 0))
    open_count = int(_num(current_open_positions, 0))
    trades = int(_num(closed_trades, 0))
    wr = _num(win_rate, 0.0)
    ar = _num(avg_r, 0.0)
    established = trades >= 75
    positive_edge = wr >= 0.55 and ar >= 0.50
    strong_edge = trades >= 125 and wr >= 0.58 and ar >= 0.85
    if established and positive_edge:
        recommended = 12 if current >= 10 else 10
        action = "raise_to_10_or_12_in_steps"
    else:
        recommended = current
        action = "hold_current_capacity_until_more_edge"
    return {
        "current_max": current,
        "current_open_positions": open_count,
        "closed_trades": trades,
        ...
        "safe_to_12": bool(established and positive_edge),
        "safe_to_15": bool(strong_edge),
        "recommended_next_max": recommended,
...
def dashboard(request: Request):
    ...
    def _snapshot_display(v, fallback="snapshot unavailable"):
        return v if v not in (None, "") else fallback

    def _snapshot_value(v, fallback="snapshot unavailable"):
        return v if v not in (None, "") else fallback

    def _strategy_name_from_row(row: dict) -> str:
        return str(row.get("strategy_name") or row.get("strategy") or row.get("signal") or row.get("entry_signal") or "unknown").strip() or "unknown"

    def _aggregate_strategy_from_closed(rows: list[dict]) -> dict:
        out = {}
        for item in rows:
            row = _sd(item)
            name = _strategy_name_from_row(row)
            bucket = out.setdefault(name, {"closed_trades": 0, "wins": 0, "losses": 0, "flat": 0, "gross_pnl": 0.0, "_r_sum": 0.0, "_r_count": 0})
            bucket["closed_trades"] += 1
            pnl = _first(row, "realized_pnl", "pnl", "gross_pnl", "profit_loss")
            pnl_f = _flt(pnl) if pnl not in (None, "") else 0.0
            bucket["gross_pnl"] += pnl_f
            if pnl_f > 0: bucket["wins"] += 1
            elif pnl_f < 0: bucket["losses"] += 1
            else: bucket["flat"] += 1
            r_val = _first(row, "r", "r_multiple", "realized_r")
            if r_val not in (None, ""):
                bucket["_r_sum"] += _flt(r_val)
                bucket["_r_count"] += 1
        for b in out.values():
            ct = max(1, int(b.get("closed_trades") or 0))
            b["win_rate"] = float(b.get("wins") or 0) / float(ct)
            rc = int(b.get("_r_count") or 0)
            b["avg_r"] = (float(b.get("_r_sum") or 0.0) / float(rc)) if rc > 0 else 0.0
        return out
    ...
    active_rows = "".join(
        ...
        f"<td>{_dashboard_fmt(_snapshot_display(r['last']))}</td>"
        f"<td>{_money(r['upl']) if r['upl'] not in (None, '') else 'snapshot unavailable'}</td>"
    ...
    flat = int(sum(int(_sd(b).get("flat") or 0) for b in strategy_perf.values()))
    gross_pnl = round(sum(_flt(_sd(b).get("gross_pnl")) for b in strategy_perf.values()), 4)
    avg_r = round(sum(_flt(_sd(b).get("avg_r")) * int(_sd(b).get("closed_trades") or 0) for b in strategy_perf.values()) / max(1, closed_trades), 4)
    ...
    swing_capacity_advisory = _swing_capacity_advisory(
        current_max=_effective_max_open_positions(),
        current_open_positions=len(merged),
    ...
    if dashboard_full:
        ...
        trade_quality = {
            "open_book_risk": {
                "open_positions": len(merged),
                "max_open_positions": _effective_max_open_positions(),
                "total_notional": snap.get("total_notional") or snap.get("portfolio_notional") or "",
                "total_risk_to_stop": snap.get("total_risk_to_stop") or "",
                "total_unrealized_pl": round(total_unrealized, 2),
                "portfolio_exposure_cap_pct": _effective_portfolio_exposure_cap_pct(),
                "symbol_exposure_cap_pct": _effective_symbol_exposure_cap_pct(),
            },
    ...
    relaxed_symbols = {str(_sd(r).get("symbol") or "").upper() for r in relaxed_unique if str(_sd(r).get("symbol") or "").strip()}
    correlation_proxy_rows = [
        _sd(r) for r in closed_trade_rows
        if str(_sd(r).get("symbol") or "").upper() in relaxed_symbols
    ]
    correlation_proxy_closed = len(correlation_proxy_rows)
    correlation_proxy_wins = sum(1 for r in correlation_proxy_rows if _flt(_first(r, "realized_pnl", "pnl", "gross_pnl", "profit_loss")) > 0)
    correlation_proxy_pnl = round(sum(_flt(_first(r, "realized_pnl", "pnl", "gross_pnl", "profit_loss")) for r in correlation_proxy_rows), 2)
    correlation_proxy_win_rate = round((100.0 * correlation_proxy_wins / max(1, correlation_proxy_closed)), 2) if correlation_proxy_closed else 0.0
    ...
    for scan in recent_scan_history[-rejection_scan_window:]:
        scan_row = _sd(scan)
        scan_candidates = _sl(scan_row.get("candidates") or scan_row.get("results") or scan_row.get("last_swing_candidates") or _sd(scan_row.get("summary")).get("top_candidates"))
        for item in scan_candidates:
            row = _sd(item)
            reasons = [str(x) for x in _sl(row.get("rejection_reasons") or row.get("reasons")) if str(x).strip()]
            if not reasons:
            ...
            for reason in reasons:
                rejection_window_counts[reason] = rejection_window_counts.get(reason, 0) + 1
            if all(reason in correlation_gate_codes for reason in reasons):
                correlation_only_rows.append(row)
    dominant_recent_blocker = sorted(rejection_window_counts.items(), key=lambda kv: (-kv[1], kv[0]))[0][0] if rejection_window_counts else "none"
    rejection_window_html = "".join(f"<tr><td>{html.escape(str(r))}</td><td>{c}</td></tr>" for r, c in sorted(rejection_window_counts.items(), key=lambda kv: (-kv[1], kv[0]))[:8]) or '<tr><td colspan="2" class="muted">No recent rejection history available.</td></tr>'
    correlation_unique = {}
    for row in correlation_only_rows:
        sym = _up(row.get("symbol"))
        if sym and (sym not in correlation_unique or _flt(row.get("rank_score")) > _flt(_sd(correlation_unique.get(sym)).get("rank_score"))):
            correlation_unique[sym] = row
    correlation_preview = sorted(correlation_unique.values(), key=lambda r: _flt(_sd(r).get("rank_score")), reverse=True)[:10]
    correlation_preview_html = "".join(f"<tr><td>{html.escape(_up(_sd(r).get('symbol')))}</td><td>{_dashboard_fmt(_sd(r).get('rank_score'))}</td><td>{_dashboard_fmt(_sd(r).get('close'))}</td><td>{html.escape(', '.join(_sl(_sd(r).get('rejection_reasons') or _sd(r).get('reasons'))[:2]))}</td></tr>" for r in correlation_preview) or f'<tr><td colspan="4" class="muted">No correlation-only blocks in this window; dominant blocker is {html.escape(str(dominant_recent_blocker))}.</td></tr>'
    ...
    if halt_loss_control:
    ...
    elif not scanner_ready or trading_blocked:
        ...
        halt_panel_class = "neutral" if scanner_in_flight and not trading_blocked else "bad"
    else:
...
# Reconcile first: never place entries against stale internal state.
        ...
        def _eval_one(sym: str) -> dict:
                ...
                try:
                    ...
                    diag = {
                        ...
                        'intraday_vwap_continuation': {'enabled': bool(INTRADAY_VWAP_CONTINUATION_ENABLE and STRATEGY_MODE == "intraday")},
                    }
                    ...
                    if _hard_market_closed:
                        ...
                        for _k in ('midbox', 'pwr', 'vwap_pullback', 'intraday_vwap_reclaim', 'intraday_vwap_continuation'):
                    ...
                    if bars_today:
                        ...
                        if INTRADAY_VWAP_CONTINUATION_ENABLE and STRATEGY_MODE == "intraday" and (not _hard_market_closed) and diag.get('intraday_vwap_continuation', {}).get('eligible', True):
                            diag["intraday_vwap_continuation"].update(_intraday_vwap_continuation_setup(bars_today))
                    ...
                    if not plan and action == "hold":
                        ...
                        if use_vwap_only:
                            ...
                            if not signal_name and INTRADAY_VWAP_CONTINUATION_ENABLE and STRATEGY_MODE == "intraday" and (not _hard_market_closed) and diag.get('intraday_vwap_continuation', {}).get('eligible', True):
                                continuation, continuation_diag = eval_intraday_vwap_continuation_signal_with_diag(bars_today)
                                if isinstance(continuation_diag, dict):
                                    diag["intraday_vwap_continuation"].update(continuation_diag)
                                if continuation == "BUY":
                                    signal_name, side = ("INTRADAY_VWAP_CONTINUATION", "buy")
                        else:
                            hf_sig = None
                        ...
                        if signal_name and side in ("buy", "sell"):
                            ...
                            if str(signal_name or "").upper() == "INTRADAY_VWAP_RECLAIM":
                                signal_diag = diag.get("intraday_vwap_reclaim", {})
                            elif str(signal_name or "").upper() == "INTRADAY_VWAP_CONTINUATION":
                                signal_diag = diag.get("intraday_vwap_continuation", {})
                            else:
                                signal_diag = diag.get("vwap_pullback", {})
        ...
        strat_counts: dict[str, Counter] = {
            ...
            "intraday_vwap_continuation": Counter(),
        }
        for r in results:
        ...
        for r in results:
            try:
                ...
                if continuation:
                    continuation_rank_score, continuation_rank_meta = compute_signal_rank("INTRADAY_VWAP_CONTINUATION", continuation)
                    pre_ranked_candidates.append({
                        "symbol": r.get("symbol"),
                        "reason": continuation.get("reason"),
                        "signal_family": continuation_rank_meta.get("family"),
                        "raw_score": continuation_rank_meta.get("raw_score"),
                        "rank_score": continuation_rank_meta.get("rank_score"),
                        "dist_to_vwap_pct": continuation.get("dist_to_vwap_pct"),
                        "candidate_path": "intraday_vwap_continuation",
                    })
                ...
                continuation_nm = continuation.get("near_miss") or {}
                if bool(continuation_nm.get("near")):
                    near_miss_symbols.append({
                        "symbol": r.get("symbol"),
                        "score": continuation.get("score"),
                        "dist_to_vwap_pct": continuation.get("dist_to_vwap_pct"),
        ...
        if signals:
            for plan in signals:
                ...
                min_rank = _scanner_min_rank_for_signal(plan.get("signal", ""), family)
