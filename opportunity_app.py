...
from opportunity_lab.weather_backtest import (bias_corrected_walk_forward_dallas, historical_dallas_backtest,
                                              walk_forward_dallas)
...
APP_VERSION = "opportunity-lab-web-v10"
...
document.getElementById('weatherBiasRun').onclick=()=>post('/diagnostics/opportunity_lab/weather/dallas/bias-corrected',{days:60,lead_days:1});
...
@app.post("/diagnostics/opportunity_lab/coinbase/cost-recovery-carry")
def coinbase_cost_recovery_carry(body: dict) -> dict:
    days = max(180, min(365, int(body.get("days") or 365)))
    total_cost_bps = max(0.0, min(1000.0, float(body.get("total_cost_bps") if body.get("total_cost_bps") is not None else 139.0)))
    end = datetime.now(timezone.utc).replace(minute=0, second=0, microsecond=0) - timedelta(seconds=1)
    start = end - timedelta(days=days)
    futures, future_transport = fetch_product_candles("ETP-20DEC30-CDE", start=start, end=end)
    ...
    return {"ok": True, "market": "ETH", "requested_days": days, "future_transport": future_transport,
            "spot_transport": spot_transport,
            "research": cost_recovery_carry_walk_forward(futures, spots, total_cost_bps=total_cost_bps),
            "execution_enabled": False}
...
@app.post("/diagnostics/opportunity_lab/weather/dallas/bias-corrected")
def weather_dallas_bias_corrected(body: dict) -> dict:
    days = max(30, min(90, int(body.get("days") or 60)))
    lead_days = max(1, min(7, int(body.get("lead_days") or 1)))
    result = bias_corrected_walk_forward_dallas(days=days, lead_days=lead_days)
    if not result.get("ok"):
        raise HTTPException(status_code=502, detail=result)
    return result
