...
from opportunity_lab.sports_research_models import outcomes, fit_prior, tennis_probability, estimate
from opportunity_lab.sports_research_mapping import match
from opportunity_lab.sports_paper_research import Archive, fresh, open_paper, build_observation
...
def test_paper_cannot_enable_real_execution(tmp_path):
    with pytest.raises(ValueError, match='Paper-only'):
        open_paper(tmp_path/'paper.db', {'version': 'sports_paper_research_1.0', 'execution_enabled': True})
...
def test_ledger_restart_preserves_accounts_and_rejects_protocol_change(tmp_path, monkeypatch):
    import opportunity_lab.sports_paper_research as research
    monkeypatch.setattr(research, 'ROOT', tmp_path)
    source = tmp_path/'source.py'; source.write_text('paper only')
    protocol = {'version':'sports_paper_research_1.0','execution_enabled':False,
