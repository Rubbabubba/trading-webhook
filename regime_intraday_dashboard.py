...
def _render_detailed_dashboard(*, scan: dict, ledger: dict, readiness: dict, scanner: dict) -> str:
    ...
    order_rows = "".join(
        f"<tr><td>{_esc(signal_id)}</td><td>{_esc(row.get('status'))}</td><td>{_esc(row.get('order_id'))}</td><td>{_esc('sent' if row.get('entry_submitted_email_sent') else row.get('entry_submitted_email_error') or 'pending')}</td><td>{_esc('sent' if row.get('entry_filled_email_sent') else row.get('entry_filled_email_error') or 'pending')}</td><td>{_esc(dict(row.get('exit_decision') or {}).get('reason'))}</td><td>{_esc(dict(row.get('close_order') or {}).get('status'))}</td></tr>"
        for signal_id, row in orders.items()
    ) or "<tr><td colspan='7' class='muted'>No paper orders recorded.</td></tr>"
    ...
    shadow_summary = {
        ...
        "research_shadow_only": sum(row.get("submission_status") == "research_shadow_only" for row in signal_shadows),
    ...
    ) or "<tr><td colspan='8' class='muted'>No strategy paper entries have been submitted.</td></tr>"
...
<div class='top'><div><div class='pill'>PAPER ONLY — LIVE INTRADAY CLOSED</div><h1>Regime-Intraday Console</h1><div class='muted'>Generated {generated}. Auto-refresh 30 seconds. SPY and DIA are broker-paper sleeves. QQQ and IWM are shadow research only and cannot reach option selection or order submission. Exit requests are not fills: confirm remaining positions in Alpaca paper.</div></div><div><a href='/health'>System health</a></div></div>
