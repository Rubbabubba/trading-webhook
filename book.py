...
def _sma(arr: np.ndarray, n: int) -> float:
    if n <= 1 or len(arr) < n:
        return float("nan")
    return float(pd.Series(arr).rolling(n).mean().to_numpy()[-1])

def _atr_from_5m(five: dict, n: int = 14) -> float:
    return float(_atr(five["high"], five["low"], five["close"], n)[-1])

def sig_e1_vwap_bos(
    one: dict,
    five: dict,
    *,
    min_dev_atr: float = 1.5,
    bos_lookback: int = 10,
    vol_sma_n: int = 9,
    vol_mult: float = 1.0,
    ...
    def roll_max(arr, n): return pd.Series(arr).rolling(n).max().to_numpy()
    def roll_min(arr, n): return pd.Series(arr).rolling(n).min().to_numpy()

    rhi = roll_max(high5, bos_lookback)
    rlo = roll_min(low5, bos_lookback)

    def find_recent_bos(dir_: str):
        for k in range(0, retest_lookback):
            j = i - k
            if j - 1 <= 0:
                break
            ph = float(rhi[j-1])
