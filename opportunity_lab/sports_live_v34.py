...
from .sports_live_books_v33 import Book, credentials, stream_loop
from .sports_live_load import LatestBooks, RateBudget, fresh_stream
...
def scoreboard_dates(now,league):
    return ((now-timedelta(days=1)).strftime('%Y%m%d')+'-'+(now+timedelta(days=1)).strftime('%Y%m%d')) if league in ('epl','mls') else now.strftime('%Y%m%d')
...
def run(manifest, output, once=False):
    ...
    inbox=LatestBooks();stop=threading.Event();selected=set()
    publish=inbox.put
    ...
    def reports():
        from .sports_live_report_current34 import report
    ...
    stream_state='starting';next_meta=next_tick=next_status=next_book=0;backoff_until=0;errors={};service_gap={};book_queue=[];budget=RateBudget();last_integrity=0;stream_incidents=0;max_handoff_age=0;next_expire=0;next_settlement_slot=0
    try:
        while utcnow()<timestamp(config['stop_at']):
            ...
            for item in inbox.drain():
                if item['kind']=='stream_state':
                    ...
                    if stream_state=='disconnected':stream_incidents+=1
            ...
            if stream_messages:
                # Persist complete coalesced checkpoints; no claim of lossless event capture.
                ...
                for ticker,item in updates.items():
                    max_handoff_age=max(max_handoff_age,max(0,time.time()-item['at']))
            ...
            for future in list(jobs):
                ...
                if kind in ('book','settlement') and ('429' in str(response.get('error')) or response.get('status')==429):
                    budget.limited(mono);errors['rate_limit']='kalshi_only_backoff'
                try:
                    if response.get('error') or response.get('status')!=200:raise ValueError('http_transport_failure')
                    ...
                    if kind in ('book','settlement'):
                        budget.success();errors.pop('rate_limit',None)
                    if kind=='book':
                        rtt.add(response['duration'])
                        if fresh_stream(books.get(key),time.time()):
                            errors.pop(kind+':'+key,None);continue
                        payload=record['data']['orderbook_fp'];book=Book()
                        received=timestamp(response['received_at']).timestamp()
                        book.snapshot(payload.get('yes_dollars') or [],payload.get('no_dollars') or [],received,rid)
                    ...
                    if kind=='book' and key in books and not fresh_stream(books[key],time.time()):books[key]['valid']=False
                ...
                for path in config['metadata_paths']:
                    try:
                        ...
                        for t,c in data.items():
                            if t not in held and c['game']['league']+'_'+c['game']['event_id'] not in eligible:continue
            ...
            if not paused and len(jobs)<10:
                # One shared Kalshi budget; game feeds never inherit Kalshi backoff.
                settlement_due=any(epoch-last_feed.get("settlement:"+t,0)>=60 and ("settlement",t) not in inflight for t in held)
                if budget.ready(mono) and not (settlement_due and mono>=next_settlement_slot):
                    warmup=rtt.estimate() is None
                    targets=[t for t in selected if ('book',t) not in inflight and
                             (not fresh_stream(books.get(t),epoch) or warmup or mono-last_integrity>=30)]
                    candidates=polling_batch(targets,held,last,epoch,4)
                    if candidates:
                        t=candidates[0]
                        jobs[pool.submit(request,BASE_URL+'/markets/'+t+'/orderbook?depth=20')]=('book',t)
                        last[t]=epoch;last_integrity=mono;budget.reserve(mono)
                due_feeds=[]
                for s in eligible:
                    ...
                    if league in ('atp','wta','epl','mls'):
                        date=scoreboard_dates(now,league)
                ...
                for t in held:
                    key='settlement:'+t
                    if budget.ready(mono) and epoch-last_feed.get(key,0)>=60 and ('settlement',t) not in inflight and len(jobs)<10:
                        jobs[pool.submit(request,BASE_URL+'/markets/'+t)]=('settlement',t);last_feed[key]=epoch;budget.reserve(mono);next_settlement_slot=mono+5
            if mono>=next_tick:
                ...
                if mono>=next_expire:
                    expire_markouts(db,epoch);next_expire=mono+5
                db.commit();next_tick=mono+1
                if mono>=next_status:
                    write_json(output/'status.json',{'at':utcnow().isoformat(),'version':VERSION,'execution_enabled':False,
                        ...
                        'load':{**inbox.stats(),'stream_disconnects':stream_incidents,'max_handoff_age_seconds':max_handoff_age,
                            'fresh_selected_books':sum(fresh_stream(books.get(t),time.time()) for t in selected),
                            'kalshi_429_incidents':budget.incidents,'kalshi_backoff_remaining':max(0,budget.until-mono)},
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--manifest',default='configs/sports_live_v34_20260913/manifest.json')
    p.add_argument('--output',default='sports_paper/live_v34_20260913');p.add_argument('--once',action='store_true')
