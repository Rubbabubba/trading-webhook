...
from .sports_live_books import Book, credentials, stream_loop
from .sports_live_coverage import core_markets, polling_batch, admission
from .sports_live_sync import audit
...
def run(manifest, output, once=False):
    ...
    def reports():
        from .sports_live_report import report
                ...
                if item['kind']=='stream_state':
                    ...
                    if stream_state not in ('connected','credentials_missing','no_targets'):
                        for b in books.values():
                            if b.get('transport')=='websocket':b['valid']=False
                else:
                    raw=json.dumps(item['message']).encode()
                    response={'raw':raw,'received_at':datetime.fromtimestamp(item['at'],timezone.utc).isoformat(),
                              'status':200,'error':None,'headers':{},'duration':0,'url':'kalshi_websocket'}
                    persist(db,now.isoformat(),item['ticker'],'stream',response)
                    rid=db.execute('select last_insert_rowid()').fetchone()[0]
                    previous=books.get(item['ticker']);service_gap[item['ticker']]=item['at']-previous['at'] if previous else 999
                    books[item['ticker']]={'at':item['at'],'quote':item['quote'],'valid':True,'id':rid,'transport':'websocket'}
                    update_markouts(db,item['ticker'],item['at'],market_view(item['ticker'],cache,books,utcnow()))
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--manifest',default='configs/sports_live_v3_20260912/manifest.json')
    p.add_argument('--output',default='sports_paper/live_v3_20260912');p.add_argument('--once',action='store_true')
