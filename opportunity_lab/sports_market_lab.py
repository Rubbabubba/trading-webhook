    ...
    try:
        while utcnow() < timestamp(config['stop_at']):
            ...
            if not (output/'PAUSE').exists():
                ...
                selected = (due[offset:]+due[:offset])[:200]
...
if __name__ == '__main__':
    p=argparse.ArgumentParser();p.add_argument('--manifest',default='configs/sports_market_lab_20260910/manifest.json');p.add_argument('--output',default='sports_paper/market_lab_20260910');p.add_argument('--once',action='store_true');a=p.parse_args();run(a.manifest,a.output,a.once)
