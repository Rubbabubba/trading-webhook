...
from .sports_research_models_v11 import estimate, fit_prior, identity, participants, score
from .sports_research_execution import initial, decide

VERSION = 'sports_paper_research_1.1'
...
def open_paper(path, protocol):
    ...
    db.executescript('''
        ...
        CREATE INDEX IF NOT EXISTS actions_sample_horizon ON actions(sample_id,horizon);
    ''')
    return db
...
def soccer_sources_agree(competition, summary, config):
    if not identity(competition, config) or not identity(summary, config):
        return False
    kinds = [c.get('status', {}).get('type', {}) for c in (competition, summary)]
    if kinds[0].get('state') != kinds[1].get('state'):
        return False
    pregame = all(k.get('state') == 'pre' and not k.get('completed') for k in kinds)
    left, right = participants(competition), participants(summary)
    for side in ('home', 'away'):
        a, b = left[side].get('score'), right[side].get('score')
        if pregame and (a is None or b is None):
            # Missing scheduled scores are normal; a supplied nonzero score is not.
            if any(value is not None and score(value) != 0 for value in (a, b)):
                return False
            continue
        if score(a) != score(b):
            return False
...
def drain_report(db, pending, output):
    """Bound reporting to one queued game per decision cycle."""
    if pending:
        slug = next(iter(pending))
        del pending[slug]
...
def build_observation(config, archives, anchor, memory, now):
    ...
    if league not in ('atp', 'wta'):
        ...
        if league in ('epl', 'mls'):
            try:
                if not soccer_sources_agree(competition, summary_comp, config):
        ...
        while utcnow() < timestamp(protocol['stop_at']):
            ...
            if time.monotonic() >= next_discovery:
                ...
                for config in configs:
                    if config['league'] + '_' + config['event_id'] not in protocol['evaluation_slugs']:
                        continue
                    try:
            ...
            try:
                drain_report(db, pending_reports, output)
            except Exception as exc:
                write_json(output / 'report_error.json', {'at': utcnow().isoformat(), 'error': str(exc)[:300]})
            if report_due:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--manifest', default='configs/sports_paper_research_v11_20260912/manifest.json')
    parser.add_argument('--output', default='sports_paper/research_v11_20260912')
