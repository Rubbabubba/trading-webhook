...
def report(output):
    ...
    try:source.backup(db,pages=256,sleep=.01)
    ...
    try:
        ...
        for slug,raw,anchor,memory,state in db.execute('select * from games').fetchall():
            ...
            lines=['# '+c['game'],'',f"Paper v3.2; {state}. Report at {result['at']}.",'',
        ...
        write_json(output/'report.json',{'at':utcnow().isoformat(),'version':'sports_live_3.2','games':summaries})
    finally:db.close()
