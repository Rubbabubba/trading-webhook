...
def select_batch(ordered, pending):
    priority = [t for t in ordered if t in pending]
    other = [t for t in ordered if t not in pending]
    # Existing probes get fresh quotes before expanding the sample.
    return (priority + other)[:200]
    ...
    try:
        while utcnow() < timestamp(config['stop_at']):
            ...
            if not (output/'PAUSE').exists():
                ...
                selected = select_batch(ordered, pending)
...
if __name__ == '__main__':
    p=argparse.ArgumentParser();p.add_argument('--manifest',default='configs/sports_market_lab_v11_20260910/manifest.json');p.add_argument('--output',default='sports_paper/market_lab_v11_20260910');p.add_argument('--once',action='store_true');a=p.parse_args();run(a.manifest,a.output,a.once)
