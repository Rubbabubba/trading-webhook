...
def report(output):
    output=Path(output)
    report_root=output/"manual_reports"
    ...
    try:
        ...
        for slug,raw,anchor,memory,state in db.execute('select * from games').fetchall():
            ...
            path=report_root/'games'/slug;path.mkdir(exist_ok=True,parents=True);write_json(path/'report.json',result)
            lines=['# '+c['game'],'',f"Paper v3.4; {state}. Report at {result['at']}.",'',
        ...
        write_json(report_root/'report.json',{'at':utcnow().isoformat(),'version':'sports_live_3.4','games':summaries})
    finally:db.close()
