...
def estimate(config, summary, competition, reference, anchor, now):
    ...
    active_phases = {'STATUS_IN_PROGRESS'}
    if league in ('epl', 'mls'):
        active_phases.update({'STATUS_FIRST_HALF', 'STATUS_SECOND_HALF'})
    if result['completed'] or kind.get('state') != 'in' or kind.get('name') not in active_phases:
