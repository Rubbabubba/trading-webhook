"""Deterministic cash-account simulation. Contains no networking or order API."""
import math
...
def entry(market, probability, cash, margin, staged=None):
    if not available(market) or probability is None or not math.isfinite(probability) or not 0 <= probability <= 1:
        return None, 'unavailable_quote_or_probability'
    q = market['quote']
    if q['ask'] - q['bid'] > .04 + 1e-9:
    ...
    for count in range(10, 0, -1):
        ...
        if buy['total'] > min(cash, 1000):
            ...
            continue
        if buy['total'] <= 0 or (buy['total'] - sell['total']) / buy['total'] > .15:
...
def decide(state, observation, now, hold_seconds, margin, paused=False):
    ...
    if pending:
        ...
        if elapsed < 10:
            return [{'action': 'wait', 'reason': 'confirmation_delay'}]
        state['pending'] = None
        ...
        if pending['action'] == 'buy' and valid_signal and not position and not state['halted']:
            chosen, reason = entry(m, model['probabilities'].get(pending['side']), state['cash_cents'], margin, pending['price'])
            if chosen:
        ...
        if pending['action'] == 'sell' and position and available(m, position['count']):
            price = min(pending['price'], m['quote']['bid'])
            proceeds = cost(price, position['count'], m['fee_coefficient'], False)
            profit = proceeds['total'] - position['cost']['total']
            ...
            state['last_exit_at'] = now.isoformat()
            mark(state, markets)
            return [{'action': 'sell', 'position': position, 'price': price, 'proceeds': proceeds,
                     'profit_cents': profit, 'reason': pending['reason']}]
        return [{'action': 'cancel', 'reason': 'confirmation_gate'}]
    if position:
        m = markets[position['side']]
        if not available(m, position['count']):
        ...
        if reason:
            state['pending'] = {'action': 'sell', 'side': position['side'], 'price': m['quote']['bid'],
                                'at': now.isoformat(), 'snapshot': observation['snapshot'], 'book_id': m['book_id'], 'reason': reason}
