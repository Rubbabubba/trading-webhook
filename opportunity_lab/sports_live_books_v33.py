    ...
    while not stop.is_set():
        ...
        try:
            async with websockets.connect(WS_URL, additional_headers=auth_headers(*creds),
                ...
                dirty=set(); last_publish=started; event_count=0; snapshot_requested={} 
                last_snapshot=started;command_id=1
                publish({'kind': 'stream_state', 'state': 'connected'})
                while not stop.is_set():
                    # Target changes are batched; healthy sessions have no periodic reconnect.
                    if time.monotonic()-started > 60 and sorted(targets()) != tickers:
                        break
                    if books.sequence and time.monotonic()-last_snapshot >= 1:
                        quiet=[t for t in tickers if time.time()-books.books.get(t, Book()).at >= 3 and time.monotonic()-snapshot_requested.get(t,0)>=3]
                        for t in quiet:snapshot_requested[t]=time.monotonic()
                        command_id+=1
                        if quiet:await ws.send(json.dumps({'id':command_id,'cmd':'update_subscription','params':
                            {'sid':next(iter(books.sequence)),'action':'get_snapshot','market_tickers':quiet}}))
                    ...
                    if ticker:
                        dirty.add(ticker);event_count+=1
                    if time.monotonic()-last_publish >= .25:
                        # All deltas are applied above. Audit complete observed checkpoints;
                        # intermediate events are intentionally coalesced, not claimed as raw capture.
                        for t in dirty:
                            b=books.books[t]
                            checkpoint={'type':'coalesced_book_checkpoint','ticker':t,
                                'revision':b.revision,'session_events_applied':event_count,
                                'levels':{s:[[str(p),str(n)] for p,n in rows.items()] for s,rows in b.levels.items()}}
                            publish({'kind':'stream_book','ticker':t,'at':b.at,
                                     'message':checkpoint,'quote':b.quote(),'revision':b.revision})
                        dirty.clear();last_publish=time.monotonic()
