...
$outputPath = Join-Path $repoRoot 'sports_paper/live_v3_20260912'
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($existing -and $existing.CommandLine -like '*opportunity_lab.sports_live_v3*') {
...
$paperProcess = Start-Process -FilePath (Join-Path $repoRoot '.venv/Scripts/python.exe') -ArgumentList @('-m','opportunity_lab.sports_live_v3') -WorkingDirectory $repoRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $outputPath 'paper.stdout.log') -RedirectStandardError (Join-Path $outputPath 'paper.stderr.log')
