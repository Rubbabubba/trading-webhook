...
def snapshot_ledger(path):
    ...
    try:
        # SQLite's online backup releases source read locks between page batches.
        # Analysis then holds no transaction on the writer's live ledger.
        source.backup(snapshot, pages=128, sleep=.01)
    except Exception:
        snapshot.close()
        raise
    finally:
