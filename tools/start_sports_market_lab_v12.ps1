...
$outputPath = Join-Path $repoRoot 'sports_paper/market_lab_v12_20260912'
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($existing -and $existing.CommandLine -like '*opportunity_lab.sports_market_lab_v12*') {
        Write-Output "Market laboratory already running with PID $paperId."
        exit 0
    }
}
$paperProcess = Start-Process -FilePath (Join-Path $repoRoot '.venv/Scripts/python.exe') -ArgumentList @('-m','opportunity_lab.sports_market_lab_v12') -WorkingDirectory $repoRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $outputPath 'lab.stdout.log') -RedirectStandardError (Join-Path $outputPath 'lab.stderr.log')
