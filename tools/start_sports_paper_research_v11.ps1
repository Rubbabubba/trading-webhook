...
$outputPath = Join-Path $repoRoot 'sports_paper/research_v11_20260912'
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($existing -and $existing.CommandLine -like '*opportunity_lab.sports_paper_research_v11*') {
        Write-Output "Paper research already running with PID $paperId."
        exit 0
    }
}
$paperProcess = Start-Process -FilePath (Join-Path $repoRoot '.venv/Scripts/python.exe') -ArgumentList @('-m','opportunity_lab.sports_paper_research_v11') -WorkingDirectory $repoRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $outputPath 'paper.stdout.log') -RedirectStandardError (Join-Path $outputPath 'paper.stderr.log')
